...
export function initLivingEvidence(config) {
  ...
  function proposeStudy(args) {
    ...
    const response = {
      ...
      ...(nearDup ? { possible_duplicate_of: nearDup.id } : {}),
...
`;
  ...
  async function exportDocument() {
    ...
    ledger({
      ...
      inputs: { k: studies.length, claims: claims.length },
      result: html,
      summary: `exported ${filename} — self-contained document, ${bytes} bytes, k=${studies.length}, ${claims.length} claim(s)`,
    });
    return {
      filename, bytes, html, downloaded,
      note: 'A complete Living Evidence document: HTML + data + statistics engine + WebMCP tools in one file. It runs from file:// with no network access.',
    };
  }
  ...
  const tools = [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          quote: { type: 'string', description: 'short verbatim excerpt the extracted numbers were read from (required)' },
        },
    ...
    },
    {
      ...
      description: 'Return the append-only audit ledger: every analysis, claim verdict, proposal and human approval in this session, in order. Each entry is {run, time, actor (human|agent|system), kind, tool, inputs, summary, evidence_version, result_digest} — the digest is an FNV-1a fingerprint of that call\'s result payload.',
      inputSchema: { type: 'object', properties: {} },
    ...
    tools.push(
      {
        ...
        inputSchema: {
          type: 'object',
          properties: { text: { type: 'string', description: 'the hypothesis, as one sentence' } },
          required: ['text'],
        },
      ...
      {
        name: 'add_claim',
        description: 'Register a new addressable claim with its deterministic machine check. `test` is the declarative rule AST: {analysis, args?, focus?, verdicts:[{when:[{path,op,value}], verdict, reason}, …, {default:true, verdict, reason}]}. Ops: lt, le, gt, ge, eq, ne, abs_lt, abs_ge. Only the SHAPE is validated here — a claim is registered because it is well-formed, not because it currently passes. Use evaluate_claim afterwards to get a verdict.',
        inputSchema: {
          type: 'object',
          properties: {
            id: { type: 'string', description: 'optional claim id; auto-assigned (wc01, wc02, …) when omitted' },
            statement: { type: 'string', description: 'the claim in plain language, as a reader would argue about it' },
            rule: { type: 'string', description: 'human-readable description of the check ("Supported iff …")' },
            test: { type: 'object', description: 'the machine-check AST (see description)' },
          },
