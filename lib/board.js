...
export function initBoard(config) {
  ...
  const tools = [
    ...
    {
      name: 'propose_node',
      description: 'Propose adding a node to this experimental local sandbox. Human approval adds it to the graph but does not verify any source, quote, edge, or claim. label (<=80 chars) and statement are always required. Evidence also requires value, year, unverified kind, cited_as and a verbatim quote; quote_origin describes where the quoted text came from and never sets verified=true or certifies the source. Optional source_locator is stored as provenance. A duplicate label is flagged, not rejected; approved evidence remains explicitly not independently verified.',
      inputSchema: {
        oneOf: [
          {
            type: 'object',
          ...
          },
          {
            ...
            properties: {
              ...
              quote_origin: { type: 'string', enum: ['conversation', 'primary_source'], description: 'where the quoted text came from; this does not set verified=true or certify the source. Defaults to conversation.' },
