...
/** The v1 default topic before the public Board switched to an English
 *  presentation. Exact-match migration preserves every user-authored topic
 *  while preventing the old default from remaining stuck in localStorage. */
const LEGACY_JAPANESE_TOPIC = '会話で報告された東京の『専業主婦率』は、どの年・母集団・指標定義で他地域より高いのか。差が確認できる場合、経済的選抜・家族構造・時間コストはどこまで説明しうるか（一次資料未照合）';
...
const QUOTE_ORIGINS = ['conversation', 'primary_source'];
...
export function initBoard(config) {
  ...
  function evidenceFieldsValid(n) {
    ...
    return (n.quote_status === undefined || n.quote_status === null || n.quote_status === 'available')
      && typeof n.quote === 'string' && n.quote.trim().length > 0
      && (n.quote_missing_reason === undefined || n.quote_missing_reason === null)
      && (n.quote_language === undefined || n.quote_language === null
        || (typeof n.quote_language === 'string' && n.quote_language.trim().length > 0))
      && (n.quote_translation === undefined || n.quote_translation === null
        || (typeof n.quote_translation === 'string' && n.quote_translation.trim().length > 0))
  ...
  /** localStorage is editable. For restored evidence, the validated top-level
   *  citation fields are authoritative; mirror them into provenance so an
   *  exported node cannot contain two conflicting versions of the quote. */
  function normalizeRestoredNode(n) {
    if (n.type !== 'evidence') return n;
    const provenance = n.provenance && typeof n.provenance === 'object' && !Array.isArray(n.provenance)
      ? n.provenance : {};
    const quoteLanguage = n.quote_language ?? null;
    const quoteTranslation = n.quote_translation ?? null;
    const quoteOrigin = QUOTE_ORIGINS.includes(provenance.quote_origin)
      ? provenance.quote_origin : 'conversation';
    const sourceLocator = typeof provenance.source_locator === 'string' && provenance.source_locator.trim()
      ? provenance.source_locator.trim() : null;
    return {
      ...n,
      quote_status: 'available',
      quote_missing_reason: null,
      quote_language: quoteLanguage,
      quote_translation: quoteTranslation,
      provenance: {
        ...provenance,
        quote: n.quote,
        quote_status: 'available',
  ...
  function restoreBoard() {
    ...
    try {
      ...
      const approvedNodes = approvedNodesRaw
        .filter((n) => n && typeof n === 'object' && NODE_TYPES.includes(n.type) && typeof n.id === 'string' && !seedIds.has(n.id) && evidenceFieldsValid(n))
        .map(normalizeRestoredNode);
      ...
      const pendingNodes = (Array.isArray(d.pendingNodes) ? d.pendingNodes : [])
        ...
        .map((p) => ({ ...p, node: normalizeRestoredNode(p.node) }));
      ...
      if (typeof d.topic === 'string' && d.topic.trim()) {
        state.topic = d.topic === LEGACY_JAPANESE_TOPIC ? SEED.topic : d.topic;
      }
    ...
    const card = h('div', { class: 'le-pending-card', id: `le-pending-node-${n.id}` }, [
      ...
      h('table', { class: 'le-pending-table' }, [
        h('tbody', {}, [
          ...
          n.type === 'evidence' && n.quote_status === 'not_available' ? ['quote status', 'not available'] : null,
          n.type === 'evidence' && n.quote_status === 'not_available' ? ['quote missing reason', n.quote_missing_reason] : null,
          n.type === 'evidence' && n.quote_status !== 'not_available' ? [`quote (original · ${n.quote_language || 'language not recorded'})`, n.quote] : null,
          n.type === 'evidence' && n.quote_status !== 'not_available' && n.quote_translation ? ['English translation', n.quote_translation] : null,
          n.type === 'question' && n.test_sketch ? ['test_sketch', n.test_sketch] : null,
        ].filter(Boolean).map(([k, v]) => h('tr', {}, [h('th', { text: k }), h('td', { text: String(v) })]))),
      ]),
      h('div', { class: 'le-pending-note', text: 'An agent proposed this node. Accepting it adds it to this local board; it does not verify any source, quote, edge, or claim.' }),
      h('div', { class: 'le-pending-actions' }, [
        h('button', { class: 'le-btn le-btn-approve', text: 'Accept onto local board', onclick: () => { approveNode(item); card.remove(); syncPendingSection(); } }),
  ...
  function renderEdgeProposalCard(item) {
    ...
    const card = h('div', { class: 'le-pending-card', id: `le-pending-edge-${e.id}` }, [
      ...
      h('div', { class: 'le-pending-note', text: 'An agent proposed this edge. Accepting it adds the relation to this local board; it does not verify the relation or its endpoints.' }),
      h('div', { class: 'le-pending-actions' }, [
        h('button', { class: 'le-btn le-btn-approve', text: 'Accept onto local board', onclick: () => { approveEdge(item); card.remove(); syncPendingSection(); } }),
  ...
  function boardOverview() {
    ...
    return {
      page: 'Living Evidence Board — experimental appendix',
      status: 'experimental_appendix',
      scientific_evidence_status: 'unverified_seed_not_part_of_exemplar',
      approval_semantics: 'Approval accepts an item onto this local graph; it does not verify its source, quote, edge, or claim.',
      topic: state.topic,
      board_version: state.boardVersion,
      ...
      suite_context: {
        ...
        board: 'board.html — experimental appendix: an unverified conversation-to-graph sandbox; not part of the exemplar, its evidence base, or its numerical/software verification.',
      },
      ...
      honesty: [
        'Approval accepts an item onto this local graph; it does not verify its source, quote, edge, or claim.',
        'Experimental appendix: the unverified seed and local graph are not part of the Pygmalion meta-analysis, its evidence base, numerical reference checks, or software verification suite.',
  ...
  function proposeNode(args = {}) {
    ...
    if (args.type === 'evidence') {
      ...
      node.quote_status = 'available';
      node.quote_missing_reason = null;
      node.quote_language = args.quote_language === undefined || args.quote_language === null
        ? 'und'
        : requireString(args.quote_language, 'quote_language', { max: 35 }).trim();
      node.quote_translation = args.quote_translation === undefined || args.quote_translation === null
        ? null
        : requireString(args.quote_translation, 'quote_translation').trim();
    ...
    // ONLY on provenance. Validate the same enum advertised by the schema even
    // when invokeTool is called directly without a WebMCP schema validator.
    let quoteOrigin = null;
    if (args.type === 'evidence') {
      quoteOrigin = args.quote_origin === undefined || args.quote_origin === null
        ? 'conversation'
        : requireString(args.quote_origin, 'quote_origin').trim();
      if (!QUOTE_ORIGINS.includes(quoteOrigin)) {
        throw new Error(`quote_origin must be one of ${QUOTE_ORIGINS.join(', ')} (got ${quoteOrigin})`);
      }
    }
    ...
    node.provenance = {
      ...
      quote_status: node.quote_status ?? null,
      quote_missing_reason: node.quote_missing_reason ?? null,
      quote_language: node.quote_language ?? null,
      quote_translation: node.quote_translation ?? null,
      cited_as: node.cited_as ?? null, verification: node.verification ?? null,
    ...
    const response = {
      ...
      message: 'Proposal recorded and shown to the human reader with an Approve/Reject card. It is NOT on the board until approved. Ask the human to review it on the page.'
        ...
        + ' Human approval adds the node to this local graph; it does not verify any source, quote, edge, or claim. Call board_overview or list_nodes again after the human approves.',
    };
    ledger({
  ...
  function proposeEdge(args = {}) {
    ...
    const response = {
      ...
      message: 'Proposal recorded and shown to the human reader with an Approve/Reject card. It is NOT on the board until approved. Ask the human to review it on the page.'
        + ' Human approval adds the edge to this local graph; it does not verify the relation or either endpoint. Call get_edges or get_board_diagnostics again after the human approves.',
    };
    ledger({
  ...
  function renderPanel(id, detail, onBoard) {
    ...
    if (detail.type === 'evidence') {
      rows.push(
        ...
        ['cited_as', detail.cited_as],
        detail.quote_status === 'not_available' ? ['quote status', 'not available'] : null,
        detail.quote_status === 'not_available' ? ['quote missing reason', detail.quote_missing_reason] : null,
        detail.quote_status !== 'not_available' ? [`quote (original · ${detail.quote_language || 'language not recorded'})`, detail.quote] : null,
        detail.quote_status !== 'not_available' && detail.quote_translation ? ['English translation', detail.quote_translation] : null,
        ['verification', detail.verification || '(pending; approval can add this item to the board, but no verification workflow exists)'],
      );
    }
    ...
    const provRows = prov ? [
      ...
      prov.quote_origin ? ['quote_origin', prov.quote_origin] : null,
      prov.source_locator ? ['source_locator', prov.source_locator] : null,
    ...
    if (onBoard) {
      ...
      children.push(h('p', {
        ...
        text: `This ${detail.type} is ${detail.status === 'rejected' ? 'REJECTED — it will never appear on the board.' : 'still pending human acceptance — it is not yet placed on the map. Accept or reject it from its card in the Pending section below; acceptance is not verification.'}`,
      }));
    }
  ...
  const tools = [
    {
      ...
      description: 'Orientation to an experimental appendix: an unverified conversation-to-graph sandbox that is not part of the exemplar, its evidence base, or its numerical/software verification. Returns counts, approval semantics, honesty constraints and suggested tools. Call this first.',
    ...
    },
    {
      ...
      description: 'Full detail for one node: statement, provenance (original quote or an explicit machine-readable not-available status, quote language, optional English translation, cited_as and verification for evidence), edges in and out, and — for a claim — its computed evidence_edge_state. Accepts a bare id (e.g. "c-income") or a typed id (e.g. "claim:c-income").',
    ...
    {
      name: 'propose_node',
      description: 'Propose adding a node to this experimental local sandbox. Human approval adds it to the graph but does not verify any source, quote, edge, or claim. label (<=80 chars) and an English statement are always required. Evidence also requires value, year, unverified kind, cited_as and an original verbatim quote. Use quote_language for its BCP 47 language tag and quote_translation for an English translation when the quote is not English; a translation is presentation text, never a substitute for the original quote. quote_origin describes where the quoted text came from and never sets verified=true or certifies the source. Optional source_locator is stored as provenance. A duplicate label is flagged, not rejected; approved evidence remains explicitly not independently verified.',
      inputSchema: {
        oneOf: [
          {
            type: 'object',
            properties: {
              ...
              statement: { type: 'string', minLength: 1, description: 'full sentence in English' },
            },
          ...
          },
          {
            ...
            properties: {
              ...
              label: { type: 'string', minLength: 1, maxLength: 80, description: 'short label, <=80 characters' },
              statement: { type: 'string', minLength: 1, description: 'full sentence in English' },
              value: { type: 'string', minLength: 1, description: 'short datum value, e.g. "Tokyo 26.4% vs Fukui 7.3%"' },
              year: {
                oneOf: [
              ...
              quote: { type: 'string', minLength: 1, description: 'original verbatim fragment the value was read from; do not replace it with a translation' },
              quote_language: { type: 'string', minLength: 1, maxLength: 35, description: 'optional BCP 47 language tag for the original quote, e.g. "ja" or "en"; defaults to "und"' },
              quote_translation: { type: 'string', minLength: 1, description: 'optional English translation when the original quote is not English' },
              quote_origin: { type: 'string', enum: QUOTE_ORIGINS, description: 'where the quoted text came from; this does not set verified=true or certify the source. Defaults to conversation.' },
    ...
    {
      name: 'propose_edge',
      description: 'Propose an edge between nodes already on this experimental local board. Human approval adds the edge but does not verify the relation, source, quote, or endpoint claims. The endpoint/type compatibility matrix checks only structural types, not inferential validity. A still-pending endpoint and an identical existing/pending triple are rejected.',
      inputSchema: {
        type: 'object',
        properties: {
  ...
  function renderConsole() {
    ...
    const EXAMPLES = {
      ...
      propose_node: { type: 'evidence', label: 'Example: new datum', statement: 'This is an example proposed data point.', value: 'Example 12.3%', year: 2026, kind: 'survey', cited_as: 'Example source', quote: 'Example source-language quotation.', quote_language: 'en', quote_origin: 'conversation' },
      ...
      propose_edge: { from: 'e-hoiku', to: 'h-model', type: 'supports', rationale: 'Example: childcare capacity adds support for one component of the four-factor model.' },
