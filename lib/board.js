...
//     tally_status is edge bookkeeping over approved edges — not truth
//     adjudication — and every surface that shows it says so (TALLY_SCOPE).
...
// functions (validateEdgeShape, computeTally, computeDiscoveries,
 ...
 *  get_discoveries' unverified-evidence count. */
...
/** What tally_status IS, said everywhere it is shown (BOARD-SPEC.md §0/§1). */
export const TALLY_SCOPE = 'edge bookkeeping over approved edges — not truth adjudication';

/** What get_discoveries computes, said in the response that carries it. */
export const DISCOVERIES_NOTE = "computed from the board's approved nodes and edges under the validity matrix — bookkeeping over what the board contains, not an assessment of the literature.";
...
export function validateEdgeShape(type, fromType, toType) {
  ...
  if (!ok) {
    throw new Error(`invalid edge: ${type} from ${fromType} to ${toType} is not allowed by the validity matrix (${MATRIX_TEXT}).`);
  }
  return true;
}
...
/** BOARD-SPEC.md §1: a claim's tally_status is computed over evidence->claim
 *  edges of type supports/contradicts, and NOTHING else — not claim->
 *  hypothesis edges, not pending proposals. `edges` should be the board's
 *  current APPROVED edge list; `typeOf(id)` returns a node's type or
 *  undefined. Pure — no DOM, no board state, just data in.
 *
 *  The three statuses the spec enumerates leave one case undefined:
 *  contradicts>=1 with supports===0 (possible only via a future proposal;
 *  no seed claim reaches it). "unsupported" is reserved for the literal
 *  zero-evidence-edges case the spec states, so that case falls to
 *  "contested" — a claim with only disputing evidence is closer to "the
 *  board disagrees with itself about this" than to "the board has nothing
 *  to say", and it is at least as far from a clean "supported" reading. */
export function computeTally(claimId, edges, typeOf) {
  ...
  let status;
  if (supports === 0 && contradicts === 0) status = 'unsupported';
  else if (supports >= 1 && contradicts === 0) status = 'supported';
  else status = 'contested';
  return { status, supports, contradicts, scope: TALLY_SCOPE };
}
...
/** Live computation over one (nodes, edges) snapshot — both ACTIVE (seed +
 *  approved) only; pending proposals are not part of the board yet, exactly
 *  as tally_status ignores them. Pure — no DOM, no ledger, no board_version;
 *  initBoard's get_discoveries tool adds board_version to the response. */
export function computeDiscoveries(nodes, edges) {
  ...
  const contested_claims = claims.filter((c) => tallies.get(c.id).status === 'contested').map((c) => c.id);
  const unsupported_claims = claims.filter((c) => tallies.get(c.id).status === 'unsupported').map((c) => c.id);

  // Single-source: every SUPPORTING evidence node behind a claim cites the
  // same source. A claim with zero supporting evidence is not "single
  // source" — it is already counted as unsupported or contested.
  const single_source_claims = claims.filter((c) => {
    const supporters = edges
      .filter((e) => e.to === c.id && e.type === 'supports' && typeOf(e.from) === 'evidence')
      .map((e) => byId.get(e.from));
    if (supporters.length === 0) return false;
    const sources = new Set(supporters.map((ev) => citedAsOf(ev)));
    return sources.size === 1;
  }).map((c) => c.id);

  // Untested: a hypothesis counts as tested if a `tests` edge reaches it
  // directly, OR reaches a claim that itself supports/contradicts it. §2
  // says "no tests edge reaches them OR THEIR CLAIMS" — the claims a
  // hypothesis owns are exactly the claims with a supports/contradicts edge
  // into it, which is the same relation tally_status reads.
  ...
  const untested_hypotheses = hypotheses.filter((hy) => !testedHypotheses.has(hy.id)).map((hy) => hy.id);
  ...
  const unverified_evidence_count = evidenceNodes.filter((ev) => verificationOf(ev) === SEED_VERIFICATION_LABEL).length;
...
export function initBoard(config) {
  ...
  function approveNode(item) {
    ...
    item.node.provenance = { ...item.node.provenance, approved_at: new Date().toISOString() };
  ...
  function boardOverview() {
    ...
    return {
      ...
      suggested_flow: [
        ...
        `list_nodes {type: "claim"} — the ${counts.claim || 0} claims and their computed tally_status`,
        'get_discoveries — contested/unsupported/single-source claims, untested hypotheses, open questions',
        'propose_node / propose_edge with a quote — the human approves',
        'get_discoveries again — what changed',
      ],
      honesty: [
        `The board has NO statistics engine and issues NO verdicts. A claim's tally_status is ${TALLY_SCOPE}.`,
        ...
        'Mutations only through propose -> human approval. Quotes and cited_as are required for evidence nodes. Everything mutating is ledgered.',
        `Edges are constrained by a fixed validity matrix: ${MATRIX_TEXT}. propose_edge rejects anything else, naming the matrix.`,
      ],
      guidance: [
  ...
  function listNodes(a = {}) {
    ...
    return {
      ...
      nodes: nodes.map((n) => {
        ...
        return { ...n, tally_status: tally.status, tally_supports: tally.supports, tally_contradicts: tally.contradicts };
      }),
    };
  }
  ...
  function getDiscoveries() {
    const result = computeDiscoveries(activeNodes(), activeEdges());
  ...
  function proposeNode(args = {}) {
    ...
    if (args.type === 'evidence') {
      ...
      // and so does not count toward get_discoveries' unverified count.
  ...
  function proposeEdge(args = {}) {
    ...
    const response = {
      ...
      possible_duplicate_of: dup ? dup.id : null,
      message: 'Proposal recorded and shown to the human reader with an Approve/Reject card. It is NOT on the board until approved. Ask the human to review it on the page.'
        + (dup ? ` NOTE: an identical ${args.type} edge already exists (${dup.id}) — say so when you ask the human to approve this one.` : '')
        + ' Call get_edges or get_discoveries again after the human approves.',
    };
    ledger({
  ...
  const TALLY_GLYPH = { supported: '●', contested: '◐', unsupported: '○' };
  ...
  function nodeAriaText(n, typeOf) {
    ...
    if (n.type === 'claim') {
      ...
      return `${base} — ${t.status} (${t.supports} supports, ${t.contradicts} contradicts) — ${TALLY_SCOPE}`;
    }
  ...
  function contentFor(n, typeOf) {
    ...
    if (n.type === 'claim') {
      ...
      out.push(svText(TALLY_GLYPH[t.status], n.x + n.w / 2 - 12, n.y - 3, { size: 13, anchor: 'end', cls: `atlas-t board-tally-${t.status}`, weight: 700 }));
  ...
  function drawLegend(legendY, width) {
    ...
    const nodeRows = [
      ...
      `claim — pill; tally ${TALLY_GLYPH.supported} supported  ${TALLY_GLYPH.contested} contested  ${TALLY_GLYPH.unsupported} unsupported`,
  ...
  function renderPanel(id, detail, onBoard) {
    ...
    if (detail.type === 'evidence') {
      rows.push(
        ['value', detail.value], ['year', detail.year], ['kind', detail.kind],
        ['cited_as', detail.cited_as], ['quote', detail.quote],
        ['verification', detail.verification || '(proposed this session — not independently verified either)'],
      );
    }
    if (detail.type === 'claim' && onBoard) {
      rows.push(
        ['tally_status', `${TALLY_GLYPH[detail.tally_status]} ${detail.tally_status}`],
  ...
  const tools = [
    ...
    },
    {
      ...
      description: 'List the board\'s nodes, optionally filtered by type. Pending proposals are included inline with status:"pending" (there is no separate pending-listing tool). Claim nodes carry a computed tally_status (supported/contested/unsupported): edge bookkeeping over approved evidence edges, not truth adjudication.',
    ...
    },
    {
      ...
      description: 'Full detail for one node: statement, provenance (quote/cited_as/verification for evidence), edges in and out, and — for a claim — its computed tally_status. Accepts a bare id (e.g. "c-income") or a typed id (e.g. "claim:c-income").',
    ...
    {
      name: 'get_discoveries', readOnly: true,
      description: 'The board\'s computed discovery panel: contested claims (both supports and contradicts), unsupported claims (no evidence edges), single-source claims (all supporting evidence shares one cited_as), untested hypotheses (no tests edge reaches them or their claims), every open question with its targets, and a count of evidence still carrying the unverified seed label. Bookkeeping over approved nodes/edges under the validity matrix — never an assessment of the literature.',
      inputSchema: { type: 'object', properties: {}, additionalProperties: false },
      run: () => getDiscoveries(),
    },
    {
      name: 'propose_node',
      description: 'Propose adding a node to the board (hypothesis, claim, mechanism, evidence or question). Shown to the human as an approve/reject card — NOT on the board until approved. label (<=80 chars) and statement are always required. Evidence additionally requires value, year (integer 1900-2100 or "n/a"), kind, cited_as AND quote (the verbatim fragment the value was read from — a number without its source text is not admissible). question accepts an optional test_sketch. A (type,label) match against an existing or pending node is flagged as possible_duplicate_of, not rejected.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          value: { type: 'string', description: 'evidence only: short datum value, e.g. "東京26.4% vs 福井7.3%"' },
          year: {
            oneOf: [
              { type: 'integer', minimum: 1900, maximum: 2100 },
              { const: 'n/a' },
            ],
            description: 'evidence only: integer year (1900-2100) or the literal string "n/a"',
          },
          kind: { type: 'string', enum: EVIDENCE_KINDS, description: 'evidence only' },
          cited_as: { type: 'string', description: 'evidence only: the source name as cited in the conversation (required)' },
          quote: { type: 'string', minLength: 1, description: 'evidence only: verbatim fragment the value was read from (required)' },
          test_sketch: { type: 'string', description: 'question only: how it could be answered (optional)' },
        },
    ...
    {
      name: 'propose_edge',
      description: 'Propose adding an edge between two nodes ALREADY on the board (seed or previously approved — a still-pending node cannot be an edge endpoint yet). Shown to the human as an approve/reject card — NOT on the board until approved. Validated against the fixed matrix: supports/contradicts (evidence->claim or claim->hypothesis), part-of (mechanism->hypothesis), tests (question->claim or question->hypothesis), refines (hypothesis->hypothesis); anything else is rejected, naming the matrix. An identical (from,to,type) triple already on the board or pending is flagged as possible_duplicate_of, not rejected.',
      inputSchema: {
        type: 'object',
        properties: {
    ...
    {
      name: 'export_board',
      description: 'Export the board (nodes incl. pending, edges incl. pending, computed discoveries, audit log) as JSON. Offers the file as a browser download and returns a receipt {filename, bytes, download_started, content_digest} by default; the JSON itself is only included when include_json is true.',
  ...
  const TITLES = {
    ...
    get_edges: 'List edges', get_discoveries: 'Computed discoveries',
  ...
  function discoveryRow(label, ids) {
  ...
  function renderDiscoveries() {
    ...
    mounts.discoveries.replaceChildren(h('div', { class: 'atlas-card' }, [
      ...
      h('h3', { class: 'atlas-card-title', text: 'Computed discoveries' }),
      h('ul', { class: 'atlas-list' }, [
        discoveryRow('contested claims', d.contested_claims),
        discoveryRow('unsupported claims', d.unsupported_claims),
        discoveryRow('single-source claims', d.single_source_claims),
        discoveryRow('untested hypotheses', d.untested_hypotheses),
        h('li', {}, [h('strong', { text: `open questions (${d.open_questions.length})` }), h('span', { text: `: ${openQ}` })]),
        h('li', {}, [h('strong', { text: 'unverified evidence' }), h('span', { text: `: ${d.unverified_evidence_count} of the board's evidence nodes still carry the seed's unverified label` })]),
      ]),
  ...
  function renderConsole() {
    ...
    const EXAMPLES = {
      board_overview: {}, list_nodes: { type: 'claim' }, get_edges: {}, get_discoveries: {}, get_audit_log: {},
      get_node: { node_id: 'c-income' },
      propose_node: { type: 'evidence', label: '例: 新規データ', statement: '例として提案するデータ点。', value: '例 12.3%', year: 2026, kind: 'survey', cited_as: '例出典', quote: '例の引用文' },
      propose_edge: { from: 'e-mukyo', to: 'c-gap', type: 'supports' },
