...
const VERDICT_SCOPE = RULE_OUTCOME_SCOPE; // v0.1 response alias
const RULE_OUTCOME = { supported: 'passed', challenged: 'failed', nuanced: 'inconclusive', untested: 'not_run' };

const CELL_ID = 'cell:teacher-expectancy-iq';
const CELL_LABEL = 'Teacher expectancy → pupil IQ (experimentally induced; 18 experiments; 19 effect-size records)';
...
export function initAtlas(config) {
  ...
  function verificationGap() {
    ...
    const withSecondaryLocator = records.filter((r) => r.provenance?.source_locator).length;
    const withPrimaryCheck = records.filter((r) => r.provenance?.primary_source_checked === true).length;
    const withDerivationCheck = records.filter((r) => r.provenance?.effect_size_derivation_checked === true).length;
    const withRiskAssessment = records.filter((r) => r.risk_of_bias?.status && r.risk_of_bias.status !== 'not_assessed').length;
    return {
      id: 'gap:verification',
      type: 'verification',
      title: 'Verification gap — secondary locators present; primary-source and bias checks absent',
      count_with_manifest: withManifest,
      count_with_secondary_locator: withSecondaryLocator,
      count_with_primary_source_check: withPrimaryCheck,
      count_with_effect_size_derivation_check: withDerivationCheck,
      count_with_risk_of_bias_assessment: withRiskAssessment,
      total_records: records.length,
      statement: `All ${records.length} records have secondary metadat row locators; ${withPrimaryCheck}/${records.length} have primary-source checks, ${withDerivationCheck}/${records.length} have independent effect-size derivation checks, ${withManifest}/${records.length} have per-record data manifests, and ${withRiskAssessment}/${records.length} have structured risk-of-bias assessments.`,
      honest_framing: 'The pooled synthesis is recomputable from the yi/vi table, but that does not verify primary extraction, effect derivation, design validity, risk of bias, or data authenticity. Secondary row locators are a starting point, not primary-source traceability.',
  ...
  function cellAria() {
    ...
    return `Estimand cell. ${CELL_LABEL}. Current synthesis: ${s.spec.model}, pooled ${fmt(s.estimate)}, 95% CI ${fmt(s.ci[0])} to ${fmt(s.ci[1])}, p ${pFmt(s.p)}, I² ${fmt(s.I2, 1)}%, ${s.k} effect-size records${ex}.`;
  }
  ...
  addNode({
    ...
    detail: () => {
      ...
      return {
        ...
        experiment_count: new Set(records.map((r) => r.experiment_id || r.id)).size,
        analysis_unit: 'effect_size_record',
        unit_note: dataset.unit_note,
        synthesis: { spec: structuredClone(s.spec), estimate: s.estimate, ci: s.ci, p: s.p, tau2: s.tau2, I2: s.I2, Q: s.Q, Q_df: s.Q_df, Q_p: s.Q_p },
        claims_attached: claims.map((c) => ({
          id: c.id,
          rule_outcome: RULE_OUTCOME[claimVerdict(c.id)],
          verdict: claimVerdict(c.id),
        })),
  ...
  claims.forEach((c, i) => {
    ...
    addNode({
      ...
      aria: () => {
        ...
        const verdict = st ? st.verdict : 'untested';
        return `Claim ${c.id}: ${c.statement || c.id}. Rule outcome: ${RULE_OUTCOME[verdict]}.`;
      },
      detail: () => {
        const st = state.claimStatus.get(c.id);
        return {
  ...
  sortedRecords.forEach((r, i) => {
    ...
    addNode({
      ...
      detail: () => ({
        ...
        provenance: structuredClone(r.provenance),
        risk_of_bias: structuredClone(r.risk_of_bias),
        record_ladder_rung: 'unassigned in v0.2 — a secondary-dataset locator is present; no primary-source extraction check, approval event, data manifest, or structured risk-of-bias assessment exists for this row (see gap:verification)',
      }),
    });
  });
  ...
  function contentFor(n) {
    ...
    if (n.type === 'cell') {
      ...
      cellMetaEl = svText(`${s.k} effect-size records · I² = ${fmt(s.I2, 1)}% · ${pRel(s.p)}`, n.x, n.y + 30, { size: 9.5, cls: 'atlas-t-muted' });
  ...
  function gapMapLines(g) {
    ...
    return ['Verification gap', `${g.count_with_secondary_locator}/${g.total_records} secondary locators; ${g.count_with_primary_source_check}/${g.total_records} primary checks`, `${g.count_with_manifest}/${g.total_records} manifests · ${g.count_with_risk_of_bias_assessment}/${g.total_records} RoB assessed`];
  }
  ...
  function drawLegend() {
    ...
    let y = LEGEND.y;
    g.appendChild(svText(`Evidence: ${records.length} effect-size records / 18 experiments`, LEGEND.x, y, { size: 10, anchor: 'start', weight: 650 }));
    y += 17;
  ...
  function updateCellNode() {
    ...
    if (cellMetaEl) {
      cellMetaEl.textContent = `${s.k} effect-size records${s.spec.excluded.length ? ` (−${s.spec.excluded.length})` : ''} · I² = ${s.I2 === null ? 'n/a' : `${fmt(s.I2, 1)}%`} · ${pRel(s.p)}`;
    }
  ...
  function verdictChip(verdict) {
    const label = { supported: '✓ rule passed', challenged: '✗ rule failed', nuanced: '△ inconclusive' }[verdict] || '· rule not run';
  ...
  function cellPanel() {
    ...
    return panelCard('Estimand cell', 'Teacher expectancy → pupil IQ', [
      ...
      kv([
        ...
        ['analysis units', `${s.k} of ${records.length} effect-size records · ${new Set(studies.map((r) => r.experiment_id || r.id)).size} experiments represented · bands (all ${records.length} records): ${WEEK_BANDS.map((b) => bandCounts()[b.label]).join(' / ')}`],
        ['claims attached', claims.map((c) => `${c.id} (${RULE_OUTCOME[claimVerdict(c.id)]})`).join(', ')],
      ]),
      fig,
  ...
  function claimPanel(n) {
    ...
    return panelCard('Claim', c.id, [
      ...
      kv([
        ...
        ['about', CELL_ID],
        st ? ['rule-outcome reason', st.reason] : null,
        st ? ['evaluated at', `run #${st.run}`] : null,
      ]),
      h('details', { class: 'atlas-ast' }, [
        h('summary', { text: 'the machine-check AST an agent reads before interpreting the rule outcome' }),
  ...
  function recordPanel(n) {
    ...
    return panelCard('Evidence record', `${d.author} (${d.year})`, [
      kv([
        ...
        ['experiment id / record role', `${d.experiment_id} / ${d.record_role}`],
        ...
        ['study design / outcome / timepoint', `${d.study_design} / ${d.outcome} / ${d.timepoint}`],
        ['source', `${d.provenance.source} (${d.provenance.source_type})`],
        ['source locator', d.provenance.source_locator],
        ['derivation', d.provenance.derivation],
        ['verification status', d.provenance.verification_status],
        ['primary source checked', d.provenance.primary_source_checked ? 'yes' : 'no'],
        ['effect derivation checked', d.provenance.effect_size_derivation_checked ? 'yes' : 'no'],
        ['risk of bias', `${d.risk_of_bias.status} — ${d.risk_of_bias.note}`],
      ]),
  ...
  function evaluateClaim(rawClaimId) {
    ...
    const run = ledger({
      ...
      summary: `claim ${claimId} → REGISTERED RULE ${RULE_OUTCOME[result.verdict].toUpperCase()} (${result.reason})`,
    });
    ...
    return {
      ...
      outcome_type: 'document_registered_rule',
      rule_outcome: RULE_OUTCOME[result.verdict],
      ...
      evidence_version: EVIDENCE_VERSION,
      note: 'The registered-rule glyph is now on the claim’s node on the map, and the claim is open in the detail panel. It is not a truth or evidence-quality rating.',
    };
  }
  ...
  function synthesize({ method = 'REML', exclude = [] } = {}) {
    ...
    return {
      ...
      k_unit: 'effect_size_records',
      experiment_count_represented: new Set(included(list).map((r) => r.experiment_id || r.id)).size,
  ...
  const tools = [
    {
      ...
      run() {
        ...
        return {
          ...
          what_this_is: 'An evidence map over ONE estimand cell: 18 experiments represented by 19 Pygmalion effect-size records, plus the 6 claims the exemplar document asserts about them. Nothing here adds, removes or persists evidence; what a tool can change is the view.',
          graph: {
          ...
          cell: {
            id: CELL_ID, label: CELL_LABEL, record_count: records.length,
            experiment_count: new Set(records.map((r) => r.experiment_id || r.id)).size,
            analysis_unit: 'effect_size_record', unit_note: dataset.unit_note,
            synthesis: { spec: structuredClone(s.spec), estimate: s.estimate, ci: s.ci, p: s.p, tau2: s.tau2, I2: s.I2 },
          },
          rule_outcome_scope: RULE_OUTCOME_SCOPE,
          verdict_scope: VERDICT_SCOPE,
          suite_context: {
          ...
          honesty: [
            ...
            `A registered-rule outcome reports ${RULE_OUTCOME_SCOPE}.`,
            ...
            'All records have secondary metadat row locators; none has a primary-source extraction check, effect-derivation check, per-record data manifest, or structured risk-of-bias assessment.',
            'The source reports 18 experiments but the analysis has 19 effect-size records because Pellegrini & Hicks (1972) contributes separate aware- and blind-tester records; the reference fit does not model their within-experiment covariance.',
            'Gaps are computed from THIS collection frame. "unknown / not-searched" is a distinct state from "measured and absent".',
            'All statistics come from the page: for this fixture, selected numerical outputs (the REML fit and capped-weeks meta-regression) reproduce R metafor reference values to the tested precision. This checks numerical reproduction against the reference implementation, not the data or model assumptions, and does not validate a scientific conclusion. Use tool results when reporting page state. Independent calculations are welcome as checks — label them external and do not silently substitute them for the page’s result.',
          ],
    ...
    },
    {
      ...
      description: `The claims asserted about this cell, each with its human-readable rule, declarative machine-check AST and current registered-rule outcome. An outcome reports ${RULE_OUTCOME_SCOPE}. Legacy verdict codes remain for compatibility.`,
      inputSchema: { type: 'object', properties: {} },
      run: () => ({
        cell_id: CELL_ID,
        verdict_scope: VERDICT_SCOPE,
    ...
    {
      name: 'evaluate_claim',
      description: `Run the deterministic machine check behind one claim against the full record set. It does not inherit synthesize exclusions. The node gets a registered-rule glyph (✓ passed / ✗ failed / △ inconclusive); this is ${RULE_OUTCOME_SCOPE}. Legacy verdict codes remain in the response.`,
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          // Chrome 152 CDP drops Latin-1-only non-ASCII schemas (e.g. tau²).
          // ASCII here preserves the complete schema for native tool clients.
          method: { type: 'string', enum: ['REML', 'DL', 'FE'], description: 'tau^2 estimator / model: REML or DL random-effects, FE common-effect (default REML)' },
